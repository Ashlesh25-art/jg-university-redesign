...
export default function Navbar() {
  ...
  useEffect(() => {
    // Read stored preference only — do NOT fall back to OS preference.
    // Absence of a key means light mode (explicit opt-in for dark).
    ...
    >
      <div className="mx-auto mt-4 max-w-6xl px-4 sm:px-6">
        <div
          className={`rounded-full p-[1px] transition ${
            scrolled
              ? "bg-gradient-to-r from-blue-200/70 via-purple-200/70 to-amber-200/60 shadow-2xl shadow-blue-900/20 dark:from-blue-500/20 dark:via-purple-500/30 dark:to-amber-500/20 dark:shadow-blue-500/20"
          ...
          <div
            className={`flex items-center justify-between gap-4 rounded-full px-5 py-3 transition ${
              scrolled
                ...
                : "bg-white/10 backdrop-blur-md dark:bg-slate-900/40"
            }`}
          >
            <a
              href="#home"
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`inline-flex h-10 w-10 items-center justify-center rounded-full border transition ${
                  scrolled
                    ? "border-slate-200 bg-white/70 text-slate-700 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200"
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <motion.aside
              className="absolute right-0 top-0 h-full w-80 bg-white px-6 py-8 shadow-2xl dark:bg-slate-900"
              initial={{ x: 320 }}
              animate={{ x: 0 }}
              exit={{ x: 320 }}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 text-slate-700 dark:border-slate-700 dark:text-slate-200"
                  aria-label="Close menu"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="mt-10 flex flex-col gap-6 text-base font-medium text-slate-700 dark:text-slate-200">
                {navLinks.map((link) => (
                  <a
                    key={link.label}
                    href={link.href}
